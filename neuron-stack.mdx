...
flowchart TB
  ...
  subgraph FRAMEWORKS["ML framework integrations"]
    direction LR
    F1["Native PyTorch on Neuron<br/>TorchNeuron<br/>eager + torch.compile"]
...
- **[Native PyTorch on Neuron (TorchNeuron)](/frameworks/torch/pytorch-native-overview)** — a Neuron-specific backend for PyTorch that supports eager execution and `torch.compile`. Existing PyTorch code runs with minimal changes — primarily replacing `cuda` with `neuron`.
