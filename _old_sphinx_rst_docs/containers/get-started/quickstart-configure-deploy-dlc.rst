...
In this step, you will download a Docker image from AWS ECR. Choose one of the following options:

Option A: Use the pre-configured vLLM DLC
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

This image comes with the vLLM Neuron plugin already installed.
...
If you use this option, skip ahead to :ref:`Step 2 <quickstart_vllm_dlc_step2>`.

.. _quickstart_vllm_dlc_option_b:

Option B: Use the base PyTorch DLC and install the plugin manually
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

If you need a specific plugin version or want more control over the installation, start from the base
`pytorch-inference-neuronx container <https://github.com/aws-neuron/deep-learning-containers?tab=readme-ov-file#pytorch-inference-neuronx>`_
on `https://github.com/aws-neuron/deep-learning-containers <https://github.com/aws-neuron/deep-learning-containers>`_.
...
After starting the container (see :ref:`Step 2 <quickstart_vllm_dlc_step2>`), install the vLLM Neuron plugin from source inside the container:
