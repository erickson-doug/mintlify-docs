...
* GPT-OSS 120B (`openai/gpt-oss-120b <https://huggingface.co/openai/gpt-oss-120b>`__) and compatible EAGLE3 draft model (`nvidia/gpt-oss-120b-Eagle3 <https://huggingface.co/nvidia/gpt-oss-120b-Eagle3>`__) checkpoints in :code:`/mnt/inference/models/`
...
one Trn3 instance. You need to start each server on a unique port and assign it a specific 
range of NeuronCores. You can do that by providing them as arguments to the startup script
for example run the following, each in a separate terminal to start two copies. Since the 
script takes care of compilation, we need to provide a unique working directory if you start
up multiple servers in simultaneously.

::

    export BASE_COMPILE_WORK_DIR="/tmp/nxdi_model_8000"
    source ./neuronx_gpt_oss_120b_in_vllm_venv/bin/activate
    bash start_vllm_server.sh 8000 0-7

::
    
    export BASE_COMPILE_WORK_DIR="/tmp/nxdi_model_8001"
    source ./neuronx_gpt_oss_120b_in_vllm_venv/bin/activate
    bash start_vllm_server.sh 8001 8-15


Please refer to :ref:`/libraries/nxd-inference/tutorials/trn2-llama3.3-70b-dp-tutorial.ipynb` for more information
about data parallel inference.
