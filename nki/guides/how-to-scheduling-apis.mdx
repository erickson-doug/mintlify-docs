...
The NKI scheduling APIs provide additional control over automatic instruction scheduling. This control comes in the form of adding additional dependency edges before scheduling. The extra dependency edges constrain the reordering the automatic scheduler will do.

## Adding dependency edges

The example below shows how to specify scheduling metadata for NKI kernel functions. The extra dependency edges are communicated to the NKI compiler by setting a property on the top-level kernel functions with the scheduling edges.
...
The NKI compiler collects the data from the `with_schedule` call, checks that it makes sense, and propagates it to the scheduling pass of the compiler. The example below shows programmatic metadata generation — enforcing a sequential order for all activation operations.
...
Adding dependency edges can be tedious. To streamline the process, the NKI compiler also supports no-reorder blocks. A no-reorder block is a section of code where dependency edges are automatically added between every pair of instructions. Using no-reorder blocks, the example above can be written as:
...
The `no_reorder` block instructs the compiler to insert dependency edges between every instruction. The `no_reorder` block is *dynamically scoped*, meaning it applies to all code that would execute under the block, not just the code that's syntactically under the block. For example, the following code is equivalent:
...
Even though the `loop_body` function isn't syntactically under a `no_reorder` block, it's evaluated as a no-reorder block because the function is called from under a `no_reorder` block.
